...
function loadPeople() {
  ...
  peersUnsub = listUsersRealtime(renderPeople); // নোট: যদি এটা undefined হয়, listenUsers ব্যবহার করতে হবে
}
...
async function openChat(peer) {
  ...
  peerAvatar.src = peer.photoURL || "https://ui-avatars.com/api/?name=" + encodeURIComponent(peer.displayName || "User");
...
function renderMessages(msgs) {
  ...
  for (const m of msgs) {
    const day = dayKey(m.createdAt?.toDate?.());
    if (day !== lastDayKey) {
      lastDayKey = day;
    ...
    el.innerHTML = `
      ...
      <div class="meta">
        <span class="time">${formatTime(m.createdAt?.toDate?.())}</span>
    ...
    el.querySelectorAll(".react-btn").forEach(btn => {
      btn.addEventListener("click", () => toggleReaction(currentConvId, m.id, btn.textContent, me));
    });
    ...
    el.querySelector(".reply-btn").addEventListener("click", async () => {
      ...
      const sender = (await getUser(m.senderUid))?.displayName || "User";
